...
def add_sensor_combs(input_plans,satuBool,maxWells):
    ...
    for plan in input_plans:
        ...
        # then remove any duplicates in case two sensors are already at the same point at different depths
        existing_wells = [[deployment.wellboreSensor.i,deployment.wellboreSensor.j] for deployment in plan.deployments]
        ...
        for i,j,k in itertools.product(range(nx),range(ny),range(nz)):
            ...
            existing_sensors = [[deployment.wellboreSensor.i,deployment.wellboreSensor.j,deployment.wellboreSensor.k] for deployment in plan.deployments]
            ...
            for it in range(nt):
                if np.any(satuBool[:,i,j,k,it:]):
                    ...
                    elif (planNew.avg_ttfd(satuBool)<plan.avg_ttfd(satuBool)): keep = True
...
class MonitoringPlan:
    ...
    def __eq__(self,other):
        if self.deployments==other.deployments: return True
        else: return False
    def __hash__(self):
    ...
    def avg_ttfd(self,satuBool):
        ...
        return np.mean(ttfd)
    def drill_time(self):
        wells = [[deployment.wellboreSensor.i,deployment.wellboreSensor.j,deployment.timestep] for deployment in self.deployments]
...
class Deployment:
    ...
    def __eq__(self,other):
        if self.array==other.array and self.timestep==other.timestep and self.wellboreSensor==other.wellboreSensor: return True
        else: return False
    def __hash__(self):
        return hash((self.array,self.timestep,self.wellboreSensor))
    def to_dict(self):
        return {'timestep':self.timestep,'array':self.array,'wellboreSensor':self.wellboreSensor.to_dict()}

class WellboreSensor:
    def __init__(self,i,j,k,type):
        self.i = i
        self.j = j
        self.k = k
        self.type = type
    def __eq__(self,other):
        if self.i==other.i and self.j==other.j and self.k==other.k and self.type==other.type: return True
        else: return False
    def __hash__(self):
        return hash((self.i,self.j,self.k,self.type))
    def to_dict(self):
        return { 'i':self.i,'j':self.j,'k':self.k,'type':self.type }
...
def dict2obj(dict):
    ...
    for planDict in dict['plans']:
        ...
        for deployment in planDict['deployments']:
            ...
            type = deployment['wellboreSensor']['type']
            tt = deployment['timestep']
            wellboreSensor = WellboreSensor(i,j,k,type)
            plan.deployments.add(Deployment(timestep=tt,wellboreSensor=wellboreSensor))
...
if inputs['run_optimization']:
    ...
    for tt in range(nt):
        ...
        for iSensor in range(int(inputs['max_sensors'])):
            ...
            if iSensor==0: plans += [add_sensor_combs(input_plans,remove_iReal_tt(satuBool,iReals,tt),inputs['max_wells'])]
            else:          plans += [add_sensor_combs(np.array(plans[-1])[pareto[-1]],remove_iReal_tt(satuBool,iReals,tt),inputs['max_wells'])]
    ...
    pickle.dump({'stages':stages},open(filename,'wb'))
    filename = os.sep.join([inputs['directory_output_files'],'wellbore_results.json'])
    json.dump( {'stages':[stage.to_dict() for stage in stages]}, open(filename,'w') )
    filename = os.sep.join([inputs['directory_output_files'],'wellbore_results.yaml'])
    yaml.dump( {'stages':[stage.to_dict() for stage in stages]}, open(filename,'w') )

    filename = os.sep.join([inputs['directory_output_files'],'wellbore_results_selected.pkl'])
    pickle.dump({'selected':[stage.selected for stage in stages]},open(filename,'wb'))
    filename = os.sep.join([inputs['directory_output_files'],'wellbore_results_selected.json'])
    json.dump( {'selected':[[plan.to_dict() for plan in stage.selected] for stage in stages]}, open(filename,'w') )
    filename = os.sep.join([inputs['directory_output_files'],'wellbore_results_selected.yaml'])
    yaml.dump( {'selected':[[plan.to_dict() for plan in stage.selected] for stage in stages]}, open(filename,'w') )
...
if inputs['plot_results']:
    ...
    filename = os.sep.join([inputs['directory_output_files'],'wellbore_results.pkl'])
    stages = pickle.load(open(filename,'rb'))['stages']
    ...
    for iStage in range(len(stages)):
        ...
        #    print(iStage,iPlan,len(stages[iStage].plans[iPlan].deployments),len(stages[iStage].plans[iPlan].detections) )
    ...
    for iStage in range(len(stages)):
        ...
        for plan in np.array(stages[iStage].plans)[stages[iStage].pareto]:
            #print([[deployment.wellboreSensor.i,deployment.wellboreSensor.j,deployment.wellboreSensor.k] for deployment in plan.deployments])
            #print('iStage',iStage)
            #print('plan',plan)
    ...
    for iStage in range(len(stages)):
        ...
        for iPlan in pareto:
            ...
            for deployment in plans[iPlan].deployments:
                ...
                ax.text(3600,1750,-700+100*nSensor,'Sensor %i: %.2f [years]'%(nSensor+1,deployment.timestep*10.0),fontsize=12)
                nSensor+=1
            ...
            plt.savefig('monitoringPlan_%02i_%05i.png'%(iStage,iPlan),format='png',bbox_inches='tight')
            plt.close()

        fig = plt.figure(figsize=(12,8))
        ax = fig.add_subplot(111,projection='3d',proj_type='ortho')

        ax.scatter(det,ttfd,dtim,s=20,ec='darkgray',fc='lightgray',zorder=0,alpha=0.50)
        ax.scatter(det[pareto],ttfd[pareto],dtim[pareto],s=20,c='black',zorder=1,alpha=1.0)
        ...
        for i in range(120):
            ...
            if i<60:
                ax.view_init(az,th)
                filename = os.sep.join([inputs['directory_plots'],'wellbore_results_prelim_%02i_%03ia.png'%(iStage,i)])
                plt.savefig(filename,format='png',bbox_inches='tight')
                az -= 1
                th -= 0.15
            else:
                ax.view_init(az,th)
                filename = os.sep.join([inputs['directory_plots'],'wellbore_results_prelim_%02i_%03ib.png'%(iStage,i-60)])
                plt.savefig(filename,format='png',bbox_inches='tight')
                th += 0.75
        plt.close()
        ...
        filename = os.sep.join([inputs['directory_plots'],'wellbore_results_prelim.png'])
        plt.savefig(filename,format='png',bbox_inches='tight')
        plt.close()
        ...
        filename = os.sep.join([inputs['directory_plots'],'wellbore_results_prelim_selected_%02i.png'%iStage])
        plt.savefig(filename,format='png',bbox_inches='tight')
        plt.close()
        ...
        for iPlan in range(len(selected)):
            ...
            detected = np.any(satuBool[np.array(list(np.array(selected)[iPlan].detections)),:,:,:],axis=(0,4))
            ax.voxels(xx,yy,zz,detected,facecolors='gray',alpha=0.1)
            ...
            plt.savefig('monitoringPlan_selected_%02i_%05i.png'%(iStage,iPlan),format='png',bbox_inches='tight')
            plt.close()
