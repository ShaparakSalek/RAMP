...
def add_sensor_combs(input_plans,satuBool,nrmsBool,maxWells,maxArrays):
    ...
    for plan in input_plans:
        ...
        for iArray in range(nrmsBool.shape[1]):
            ...
            if iArray in plan.arrays: continue
...
def compute_pareto(plans,satuBool):
    ...
    dtim = np.array([plan.get_avg_drill_time() for plan in plans])
...
class MonitoringPlan:
    ...
    def get_avg_drill_time(self):
        if self.avg_drill_time is None: self.compute_avg_drill_time()
        return self.avg_drill_time
    ...
    def compute_avg_drill_time(self):
        #wells = [[deployment.well,deployment.timestep] for deployment in self.deployments]
        ...
        #self.avg_drill_time = np.mean(drill_times)
        if len(self.wells)>0: self.avg_drill_time = np.mean([well.drill_timestep for well in self.wells])
        else:                 self.avg_drill_time = 0
...
if __name__ == "__main__":
    ...
    if inputs['run_optimization']:
        ...
        #ii = np.where(nRec>10)		# include only sparse arrays (<10 receivers)
        ii = np.where(nRec<40)		# include only dense arrays (>40 receivers)
        #ii = np.where(np.multiply(nRec<10,nRec>40)) # include only medium arrays (10-40 receivers)
        nrmsBool[ii,:,:] = 0
    ...
    if inputs['plot_results']:
        ...
        print('satu,nrms shapes',satuBool.shape,nrmsBool.shape)
        ...
        print(stages)
        print(stages[0].plans)
        print(stages[0].plans[0])
        ...
        for iStage in range(len(stages)):
            ...
            for iPlan in ii:
                ...
                print([sensor.install_timestep for sensor in plans[iPlan].sensors])
                print([array.iTime for array in plans[iPlan].arrays])
                ...
                ax.voxels(xx,yy,zz,detected,facecolors='gray',alpha=0.05)

                detectable = np.any(satuBool,axis=(0,4))^detected
                ax.voxels(xx,yy,zz,detectable,facecolors='blue',alpha=0.05)

                ax.text(3500,1750,-800,'Sensors:',fontsize=14)
                ...
                if 'none' not in inputs['fixed_wells']:
                    for wellStr in inputs['fixed_wells'].split(';'):
                        ...
                        if wellStr[0].strip()=='ijk':
                            ...
                            yi = float(wellStr[2])
                            zi = float(wellStr[3])
                            ax.plot([xi,xi],[yi,yi],[-100,zi],lw=0.75,c='red')
                ...
                for sensor in plans[iPlan].sensors:
                    ...
                    j = sensor.well.j
                    k = sensor.k
                    ax.scatter(x[i],y[j],z[k],s=50,c=colors[nSensor])
                    #ax.text(3600,1750,-700+100*nSensor,'Sensor %i: %.2f [years]'%(nSensor+1,sensor.install_timestep*10.0),fontsize=12)
                    nSensor+=1

                for array in plans[iPlan].arrays:
                    for iReceiver in configuration.arrays[array.iArray]['receivers']:
                        xyzR = receivers[iReceiver]
                        ax.scatter( xyzR[0], xyzR[1]+2250, xyzR[2], s=50,c=colors[nSensor],marker='.')
                    xyzS = sources[configuration.arrays[array.iArray]['source']]
                    ax.scatter( xyzS[0], xyzS[1]+2250, xyzS[2], s=80,c=colors[nSensor],marker='^')
                    nSensor+=1
                ...
                ax.set_ylabel('Average Time to First Detection [years]',fontsize=14)
                ...
                plt.savefig('monitoringPlan_%02i_%05i.png'%(iStage,kk),format='png',bbox_inches='tight')
                plt.close()
                th += 0.75
                kk += 1
