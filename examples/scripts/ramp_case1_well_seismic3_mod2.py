...
def check_multi_source(arrays, array1, array2):
    ...
    if set(arrays[array1.iArray]['receivers']) == set(
            arrays[array2.iArray]['receivers']):
        if not arrays[array1.iArray]['source'] == arrays[
                array2.iArray]['source']:
...
def is_dominated(p, q):
    ...
    return all(p_i >= q_i for p_i, q_i in zip(p, q)) and \
        any(p_i > q_i for p_i, q_i in zip(p, q))
...
def add_sensor_combs(input_plans, satuBool, nrmsBool, maxWells, maxArrays):
    ...
    for plan in input_plans:
        ...
        for i, j, k in itertools.product(range(nx), range(ny), range(nz)):
            ...
            # if we already have a sensor of this kind at this i,j,k location,
            # we don't place another one
            existing_sensors_ijkt = [[
                sensor.well.i, sensor.well.j, sensor.k, sensor.type
            ...
            for it in range(nt):
                ...
                if np.any(satuBool[:, i, j, k, it:]):
                    ...
                    detections = np.where(
                        np.any(satuBool[:, i, j, k, it:], axis=1))[0].tolist()
                    ...
                    if len(planNew.detections.difference(plan.detections)) > 0:
                        keep = True
                    elif (planNew.get_avg_ttfd(satuBool, nrmsBool)
                          < plan.get_avg_ttfd(satuBool, nrmsBool)):
                        keep = True
                    elif len(plan.detections.difference(
                            planNew.detections)) > 0:
                        raise Exception(
                            "Error, the improved plan somehow detects fewer leaks than the plan it's based on"
                        )
...
class Stage:
    ...
    def __init__(self, *args):
        if len(args) == 1 and isinstance(args[0], dict):
            self.plans = [
                MonitoringPlan(planDict) for planDict in args[0]['plans']
            ]
            self.pareto = args[0]['pareto']
            self.iReals = args[0]['iReals']
            self.selected = [
                MonitoringPlan(planDict) for planDict in args[0]['selected']
            ]
        else:
            self.plans = args[0]
            self.pareto = args[1]
    ...
    def __eq__(self, other):
        if self.plans == other.plans and self.pareto == other.pareto and self.iReals == other.iReals and self.selected == other.selected:
            return True
        else:
            return False
    ...
    def __hash__(self):
        return hash(
            frozenset([
                frozenset(self.plans),
                frozenset(self.pareto),
                frozenset(self.iReals),
...
class MonitoringPlan:
    ...
    def __init__(self, *args):
            ...
            if 'none' not in inputs['fixed_wells']:
                for wellStr in inputs['fixed_wells'].split(';'):
                    ...
                    if wellStr[0].strip() == 'ijk':
                        ...
                        ti = float(wellStr[4])
                        file = h5py.File(
                            download_directory + '/sim%04i.h5' % 1, 'r')
        ...
        if len(args) == 1 and isinstance(args[0], dict):
            ...
            self.arrays = set(
                [SeismicArray(arrayDict) for arrayDict in args[0]['arrays']])
            self.sensors = set()
    ...
    def __eq__(self, other):
        if self.wells == other.wells and self.arrays == other.arrays and self.sensors == other.sensors:
            return True
        else:
            return False
    ...
    def __hash__(self):
        return hash(
            frozenset([
                ...
                frozenset(self.arrays),
                frozenset(self.sensors)
            ]))

    def to_dict(self):
        return {
            'wells': [well.to_dict(self.sensors) for well in self.wells],
    ...
    def get_num_leaks_detected(self):
    ...
    def compute_avg_ttfd(self, satuBool, nrmsBool):
        ...
        for detection in self.detections:
            ...
            for array in self.arrays:
                if nrmsBool[array.iArray, detection, array.iTime]:
                    ttd += [time_array[array.iTime] / 365.25]
            if len(ttd) > 0:
                ttfd += [np.min(ttd)]
        if len(ttfd) == 0:
    ...
    def compute_avg_commitment_time(self):
        commitments = []
        for array in self.arrays:
            commitments += [time_array[array.iTime] / 365.25]
        for well in self.wells:
            commitments += [times[well.drill_timestep]]
...
class Well:
    ...
    def __init__(self, *args):
        if len(args) == 1 and isinstance(args[0], dict):
            ...
            self.sensors = set([
                Sensor(sensorDict, self) for sensorDict in args[0]['sensors']
            ])
        elif len(args) == 5:
            self.i = args[0]
            self.j = args[1]
    ...
    def __eq__(self, other):
        if self.i==other.i and \
           ...
           self.drill_timestep==other.drill_timestep:
            return True
        else:
            return False
    ...
    def to_dict(self, sensors):
        ...
        return {
            ...
            'j': self.j,
            'depth': self.depth,
            'drill_timestep': self.drill_timestep,
            'sensors': sensorsThis
        }
...
class Sensor:
    ...
    def __init__(self, *args):
        if len(args) == 2 and isinstance(args[0], dict) and isinstance(
                args[1], Well):
            self.well = args[1]
            self.k = args[0]['k']
    ...
    def to_dict(self):
        return {
            'k': self.k,
            'install_timestep': self.install_timestep,
            'type': self.type
        }
...
if len(sys.argv) == 1:
    raise Exception(
        '''Please include an input argument specifying the YAML or JSON filename.
Example:
...
if isinstance(inputs['scenarios'], int):
    ...
    if '-' in inputs['scenarios']:
        scenario_indices = list(
            range(int(inputs['scenarios'].split('-')[0]),
...
if __name__ == "__main__":
    ...
    if inputs['download_data']:
        ...
        if to_unzip:
            for scen_ind in urls:
                print('Unzipping {} file ...'.format(
                    file_name.format(scen_ind)))
                path_name = os.sep.join(
                    [output_directory,
                     file_name.format(scen_ind)])
                try:
                    with zipfile.ZipFile(path_name, 'r') as zip_ref:
                        folder_to_extract = os.sep.join(
                            [output_directory,
        ...
        for resource in resources:
            if resource['name'] == fname:
                ...
                progress_bar = tqdm(total=total_size_in_bytes,
                                    unit='iB',
                                    unit_scale=True)
                with open(os.sep.join([download_directory, fname]),
                          'wb') as file:
    ...
    if inputs['download_data'] or inputs['run_optimization'] or inputs[
            'plot_results']:
        print('Step {}: Performing check of the downloaded data...'.format(
            step_ind))
        step_ind = step_ind + 1
        # Check 2nd round of downloads for incomplete data to exclude from NRMS calculations
        excluded = []
        for scen_ind in scenario_indices:
            fullpath = os.sep.join(
                    [output_directory,
                     base_name.format(scen_ind), 'data'])
        ...
        if data_case == 1:
            ...
            for scen in scenarios:
                data_setup[scen] = {
                    'folder': os.path.join('data_sim{:04}'.format(scen),
                                           'data')
                }
                for t_ind, tp in enumerate(time_points):
                    data_setup[scen]['t{}'.format(
                        t_ind + 1)] = 'data_sim{:04}_t{}.bin'.format(scen, tp)
            baseline = True
            ...
            if 'sources' in inputs.keys():
                ...
                sources = np.c_[
                    np.linspace(min_sources, max_sources, num=num_sources),
                    np.zeros(num_sources),
                    np.zeros(num_sources)]
            ...
            if 'receivers' in inputs.keys():
                ...
                receivers = np.c_[np.linspace(
                    min_receivers, max_receivers, num=num_receivers),
                                  np.zeros(num_receivers),
                                  np.zeros(num_receivers)]
        ...
        for n in range(nn):
            ...
            file = h5py.File(download_directory + '/sim%04i.h5' % scenarios[n],
                             'r')
            for it in range(12):
                #pres[n,:,:,:,it] = np.array(file['plot%i'%it]['pressure'])
                satu[n, :, :, :,
                     it] = np.array(file['plot%i' % it]['saturation']) * 100
        ...
        print('Step {}: Processing seismic data into NRMS values...'.format(
            step_ind))
        step_ind = step_ind + 1
        # Create survey with defined coordinates
        survey_config = SeismicSurveyConfiguration(sources,
                                                   receivers,
                                                   name='Test Survey')
        ...
        smt = sm.add_component_model_object(
            SeismicMonitoring(name='smt',
                              parent=sm,
                              survey_config=survey_config,
                              time_points=time_points))
        # Add keyword arguments linked to the seismic data container outputs
        smt.add_kwarg_linked_to_obs('data',
                                    dc.linkobs['seismic'],
                                    obs_type='grid')
        smt.add_kwarg_linked_to_obs('baseline',
                                    dc.linkobs['baseline_seismic'],
                                    obs_type='grid')
        # Add gridded observation
        smt.add_grid_obs('NRMS',
                         constr_type='matrix',
                         output_dir=output_directory)
        ...
        nrms = np.zeros(
            (num_scenarios, num_time_points, num_sources, num_receivers))
        ...
        for rlzn_number in range(1, num_scenarios + 1):
            ...
            data = sm.collect_gridded_observations_as_time_series(
                ...
                rlzn_number=rlzn_number
            )  # data shape (num_time_points, num_sources, num_receivers)
        ...
        if data_check:
            ...
            file_to_read = os.path.join(
                output_directory,
                'nrms_optimization_data_{}_scenarios.npz'.format(
                    num_scenarios))
        ...
        )  # (300, 20, 9, 101)  scenarios, time points, sources, receivers
        nrms = d['data']

        # Setup data that will hold results of processing NRMS data for all created arrays
        arrays_nrms = np.zeros(
            (configuration.num_arrays, num_scenarios, num_time_points,
        ...
        file_to_save = os.path.join(
            output_directory,
            'arrays_nrms_data_3max_values_{}_scenarios.npz'.format(
                num_scenarios))
        ...
        file_to_save = os.path.join(
            output_directory,
            'arrays_nrms_data_3rd_max_value_{}_scenarios.npz'.format(
                num_scenarios))
    ...
    if inputs['run_optimization']:
        ...
        pairCandidates = set()
        for pair in itertools.combinations(range(len(configuration.arrays)),
                                           2):
            if set(configuration.arrays[pair[0]]['receivers']) == set(
                    configuration.arrays[pair[1]]['receivers']):
                ...
                set1 = set(
                    np.where(np.any(nrmsBool[pair[0], :, :], axis=1))[0])
                set2 = set(
                    np.where(np.any(nrmsBool[pair[1], :, :], axis=1))[0])
                ...
                print(
                    configuration.arrays[pair[0]]['source'],
                    configuration.arrays[pair[1]]['source'],
                    np.abs(configuration.arrays[pair[0]]['source'] -
                           configuration.arrays[pair[1]]['source']))
        ...
        nRec = np.array([
            len(array['receivers']) for array in configuration.arrays.values()
        ])
        ...
        for tt in np.array(inputs['stages'].split(','), dtype='int'):
            ...
            if tt == 0:
                iReals = np.sort(
                    np.random.choice(list(range(nn)), nn // 2,
                                     replace=False)).tolist()
                input_plans = [MonitoringPlan()]
            else:
                iReals += np.sort(
                    np.random.choice(list(set(range(nn)) - set(iReals)),
                                     nn // 2 // nt,
                                     replace=False)).tolist()
            ...
            print(' ')
            print(
                'Building monitoring plans based on data avaiable in year %i of %i'
                % (tt * 10, (nt * 10)))
            plans = []
            pareto = []
            for iSensor in range(
                    int(inputs['max_sensors']) + int(inputs['max_arrays'])):

                print('Considering all viable %i-sensor monitoring plans' %
                      (iSensor + 1))
                satuTemp, nrmsTemp = remove_iReal_tt(satuBool, nrmsBool,
                                                     iReals, tt)
                if iSensor == 0:
                    plans += [
                        add_sensor_combs(input_plans, satuTemp, nrmsTemp,
                                         int(inputs['max_wells']),
                                         int(inputs['max_arrays']))
                    ]
                else:
                    plans += [
                        add_sensor_combs(
                            np.array(plans[-1])[pareto[-1]], satuTemp,
                            nrmsTemp, int(inputs['max_wells']),
                            int(inputs['max_arrays']))
                    ]
                print('...built %i monitoring plans' % len(plans[-1]))
                print('...identifying Pareto optimal plans')
                pareto += [compute_pareto(plans[-1], satuTemp)]
                print('...identified %i Pareto-optimal plans' %
                      len(pareto[-1]))
        ...
        for stage in stages:
            det = np.array([
                len(plan.detections)
                for plan in np.array(stage.plans)[stage.pareto]
            ])
            ii = np.where(det == np.max(det))[0]
            selected = np.array(stage.plans)[stage.pareto][ii]
            ttfd = np.array([
                10.0 * plan.get_avg_ttfd(satuBool, nrmsBool)
        ...
        time_dict1 = {
            index: value
            for index, value in enumerate(time_array.tolist())
        }
        time_dict2 = {
            index: value
            for index, value in enumerate(
                np.array(365.25 * times, dtype='float64').tolist())
        }
        sources_dict = {
            index: value.tolist()
        ...
        receivers_dict = {
            index: value.tolist()
            for index, value in enumerate(receivers)
        }
        ...
        for stage in stages:
            for plan in stage.plans:
                for array in plan.arrays:
                    ...
                    nRecs += [
                        len(configuration.arrays[array.iArray]['receivers'])
                    ]
        print(nRecs)
        ...
        filename = os.sep.join(
            [inputs['directory_output_files'], 'well_seismic_results.pkl'])
        pickle.dump(outputDict, open(filename, 'wb'))
        retd = outputDict.pop('nrmsBool')
        filename = os.sep.join(
            [inputs['directory_output_files'], 'well_seismic_results.json'])
        json.dump(outputDict, open(filename, 'w'), indent=4)
        filename = os.sep.join(
            [inputs['directory_output_files'], 'well_seismic_results.yaml'])
        yaml.dump(outputDict, open(filename, 'w'))
        filename = os.sep.join(
            [inputs['directory_output_files'], 'well_seismic_results.h5'])
        ...
        outputDict = {
            ...
            time_dict2,
            'selected':
            [[plan.to_dict() for plan in stage.selected] for stage in stages]
        }
        filename = os.sep.join([
            inputs['directory_output_files'],
            'well_seismic_results_selected.pkl'
        ])
        pickle.dump(outputDict, open(filename, 'wb'))
        filename = os.sep.join([
            inputs['directory_output_files'],
            'well_seismic_results_selected.json'
        ])
        json.dump(outputDict, open(filename, 'w'), indent=4)
        filename = os.sep.join([
            inputs['directory_output_files'],
            'well_seismic_results_selected.yaml'
        ])
        yaml.dump(outputDict, open(filename, 'w'))
    ...
    if inputs['plot_results']:
        ...
        filename = 'well_seismic_results.pkl'
        dir_name = './sparse_arrays'
        full_path = os.sep.join([dir_name, filename])
        file = open(full_path, 'rb')
        # dump information to that file
        data = pickle.load(file)
        # close the file
        file.close()
        nrmsBool_sparse = data['nrmsBool']
        sparse_arrays = data['seismic_arrays']

        dir_name = './medium_arrays'
        full_path = os.sep.join([dir_name, filename])
        file = open(full_path, 'rb')
        # dump information to that file
        data = pickle.load(file)
        # close the file
        file.close()
        nrmsBool_medium = data['nrmsBool']
        medium_arrays = data['seismic_arrays']

        dir_name = './dense_arrays'
        full_path = os.sep.join([dir_name, filename])
        file = open(full_path, 'rb')
        # dump information to that file
        data = pickle.load(file)
        # close the file
        file.close()
        nrmsBool_dense = data['nrmsBool']
        dense_arrays = data['seismic_arrays']

        print('Shapes', satuBool.shape, nrmsBool.shape)
        det_sparse = np.array(
            [len(plan.detections) for plan in sparse[0].plans])
        ttfd_sparse = np.array([
            plan.get_avg_ttfd(satuBool, nrmsBool_sparse) for plan in sparse[0].plans
        ])

        det_moderate = np.array(
            [len(plan.detections) for plan in moderate[0].plans])
        ttfd_moderate = np.array([
            plan.get_avg_ttfd(satuBool, nrmsBool_medium) for plan in moderate[0].plans
        ])

        det_dense = np.array([len(plan.detections) for plan in dense[0].plans])
        ttfd_dense = np.array(
            [plan.get_avg_ttfd(satuBool, nrmsBool_dense) for plan in dense[0].plans])

        plt.figure(figsize=(8, 6))

        plt.scatter(det_sparse,
                    ttfd_sparse,
                    s=10,
                    c='magenta',
                    ...
                    zorder=3,
                    alpha=1.0)
        plt.scatter(det_moderate,
                    ttfd_moderate,
                    s=15,
                    c='limegreen',
                    label='Moderate (candidates)',
                    zorder=2,
                    alpha=1.0)
        plt.scatter(det_dense,
                    ttfd_dense,
                    s=20,
                    c='cyan',
        ...
        ii = [
            int(np.where((points == item).all(axis=1))[0][0]) for item in rank
        ]
        ii = np.array(ii)[np.argsort(det_sparse[ii])].tolist()
        iPlan = ii[-2]

        plt.scatter(det_sparse[iPlan],
                    ttfd_sparse[iPlan],
                    s=10,
                    c='r',
                    label='Sparse (selected)',
                    zorder=4,
                    alpha=1.0)

        plans = moderate[0].plans
        points = np.array(list(zip(-det_moderate, ttfd_moderate)))
        rank = pareto_front_divide_and_conquer(points)
        ii = [
            int(np.where((points == item).all(axis=1))[0][0]) for item in rank
        ]
        ii = np.array(ii)[np.argsort(det_moderate[ii])].tolist()
        iPlan = ii[-2]

        plt.scatter(det_moderate[iPlan],
                    ttfd_moderate[iPlan],
                    s=15,
                    c='g',
        ...
        plans = dense[0].plans
        points = np.array(list(zip(-det_dense, ttfd_dense)))
        rank = pareto_front_divide_and_conquer(points)
        ii = [
            int(np.where((points == item).all(axis=1))[0][0]) for item in rank
        ]
        ii = np.array(ii)[np.argsort(det_dense[ii])].tolist()
        iPlan = ii[-2]

        plt.scatter(det_dense[iPlan],
                    ttfd_dense[iPlan],
                    s=20,
                    c='b',
                    label='Dense (selected)',
                    zorder=4,
                    alpha=1.0)
        ...
        detected = np.any(satuBool[
            np.array(list(np.array(plans)[iPlan].detections)), :, :, :],
                          axis=(0, 4))
        ...
        if 'none' not in inputs['fixed_wells']:
            for wellStr in inputs['fixed_wells'].split(';'):
                ...
                if wellStr[0].strip() == 'ijk':
                    ...
                    j = int(wellStr[2])
                    k = int(wellStr[3])
                    ax1.plot([x[i], x[i]], [y[j], y[j]], [-100, z[k]],
                             lw=0.75,
                             c='red')
                elif wellStr[0].strip() == 'xyz':
                    xi = float(wellStr[1])
        ...
        colors = [
            mpl.cm.jet(value) for value in np.linspace(
                0, 1,
                len(plans[iPlan].sensors) + len(plans[iPlan].arrays))
        ]
        ...
        for sensor in plans[iPlan].sensors:
            ...
            j = sensor.well.j
            k = sensor.k
            ax1.scatter(x[i], y[j], z[k], s=50, color=colors[nSensor])
        ...
        for array in plans[iPlan].arrays:
            for iReceiver in sparse_arrays[array.iArray]['receivers']:
                xyzR = receivers[iReceiver]
                ax1.scatter(xyzR[0],
                            xyzR[1] + 2250,
                            xyzR[2],
                            s=50,
                            color=colors[nSensor],
                            marker='.')
            xyzS = sources[sparse_arrays[array.iArray]['source']]
            ax1.scatter(xyzS[0],
                        xyzS[1] + 2250,
                        xyzS[2],
                        s=80,
        ...
        ax1.legend([proxy1, proxy2],
                   ['Detected Leakage', 'Undetected Leakage'])

        line = mpl.lines.Line2D([0.10, 0.35], [0.08, 0.08],
                                transform=fig.transFigure,
                                color='black',
                                linewidth=1.5,
                                zorder=0)
        fig.lines.append(line)
        fig.text(0.10 + 0.25 * 0.5,
                 0.08,
                 'Timeline [years]',
                 ha='center',
                 va='bottom',
        ...
        nSensor = 0
        for array in plans[iPlan].arrays:
            arrayTime = mpl.patches.Ellipse(
                (0.10 + 0.25 * time_array[array.iTime] / np.max(time_array),
                 0.08),
                0.005,
                0.01,
                transform=fig.transFigure,
            ...
            fig.text(0.10 +
                     0.25 * time_array[array.iTime] / np.max(time_array),
                     0.06,
                     '%i' % (time_array[array.iTime] / 365.25),
                     ha='center',
                     va='top',
                     fontsize=12,
                     color='black')
        ...
        for sensor in plans[iPlan].sensors:
            arrayTime = mpl.patches.Ellipse(
                (0.10 + 0.25 * times[sensor.install_timestep] / np.max(times),
                 0.08),
                0.005,
                0.01,
                transform=fig.transFigure,
            ...
            fig.add_artist(arrayTime)
            fig.text(0.10 +
                     0.25 * times[sensor.install_timestep] / np.max(times),
                     0.06,
                     '%i' % (times[sensor.install_timestep]),
                     ha='center',
                     va='top',
                     fontsize=12,
                     color='black')
        ...
        if 'none' not in inputs['fixed_wells']:
            for wellStr in inputs['fixed_wells'].split(';'):
                ...
                if wellStr[0].strip() == 'ijk':
                    ...
                    j = int(wellStr[2])
                    k = int(wellStr[3])
                    ax2.plot([x[i], x[i]], [y[j], y[j]], [-100, z[k]],
                             lw=0.75,
                             c='red')
                elif wellStr[0].strip() == 'xyz':
                    xi = float(wellStr[1])
        ...
        for sensor in plans[iPlan].sensors:
            ...
            j = sensor.well.j
            k = sensor.k
            ax2.scatter(x[i], y[j], z[k], s=50, color=colors[nSensor])
            #ax1.text(3600,1750,-700+100*nSensor,'Sensor %i: %.2f [years]'%(nSensor+1,sensor.install_timestep*10.0),fontsize=12)
            nSensor += 1
            #print(iReal,i,j,k,sensor.install_timestep)
            #print(np.any(satuTemp[iReal,i,j,k,sensor.install_timestep:]))
        ...
        for array in plans[iPlan].arrays:
            for iReceiver in medium_arrays[array.iArray]['receivers']:
                xyzR = receivers[iReceiver]
                ax2.scatter(xyzR[0],
                            xyzR[1] + 2250,
                            xyzR[2],
                            ...
                            color=colors[nSensor],
                            marker='.')
            xyzS = sources[medium_arrays[array.iArray]['source']]
            ax2.scatter(xyzS[0],
                        xyzS[1] + 2250,
                        xyzS[2],
                        s=80,
        ...
        ax2.legend([proxy1, proxy2],
                   ['Detected Leakage', 'Undetected Leakage'])

        line = mpl.lines.Line2D([0.40, 0.65], [0.08, 0.08],
                                transform=fig.transFigure,
                                color='black',
                                linewidth=1.5,
                                zorder=0)
        fig.lines.append(line)
        fig.text(0.40 + 0.25 * 0.5,
                 0.08,
                 'Timeline [years]',
                 ha='center',
                 va='bottom',
        ...
        for array in plans[iPlan].arrays:
            arrayTime = mpl.patches.Ellipse(
                (0.40 + 0.25 * time_array[array.iTime] / np.max(time_array),
                 0.08),
                0.005,
                0.01,
                transform=fig.transFigure,
        ...
        for sensor in plans[iPlan].sensors:
            arrayTime = mpl.patches.Ellipse(
                (0.40 + 0.25 * times[sensor.install_timestep] / np.max(times),
                 0.08),
                0.005,
                0.01,
                transform=fig.transFigure,
        ...
        if 'none' not in inputs['fixed_wells']:
            for wellStr in inputs['fixed_wells'].split(';'):
                ...
                if wellStr[0].strip() == 'ijk':
                    ...
                    j = int(wellStr[2])
                    k = int(wellStr[3])
                    ax3.plot([x[i], x[i]], [y[j], y[j]], [-100, z[k]],
                             lw=0.75,
                             c='red')
                elif wellStr[0].strip() == 'xyz':
                    xi = float(wellStr[1])
        ...
        for sensor in plans[iPlan].sensors:
            ...
            j = sensor.well.j
            k = sensor.k
            ax3.scatter(x[i], y[j], z[k], s=50, color=colors[nSensor])
            #ax1.text(3600,1750,-700+100*nSensor,'Sensor %i: %.2f [years]'%(nSensor+1,sensor.install_timestep*10.0),fontsize=12)
            nSensor += 1
            #print(iReal,i,j,k,sensor.install_timestep)
            #print(np.any(satuTemp[iReal,i,j,k,sensor.install_timestep:]))
        ...
        for array in plans[iPlan].arrays:
            for iReceiver in dense_arrays[array.iArray]['receivers']:
                xyzR = receivers[iReceiver]
                ax3.scatter(xyzR[0],
                            xyzR[1] + 2250,
                            xyzR[2],
                            ...
                            color=colors[nSensor],
                            marker='.')
            xyzS = sources[dense_arrays[array.iArray]['source']]
            ax3.scatter(xyzS[0],
                        xyzS[1] + 2250,
                        xyzS[2],
                        s=80,
        ...
        ax3.legend([proxy1, proxy2],
                   ['Detected Leakage', 'Undetected Leakage'])

        x1 = 0.70
        line = mpl.lines.Line2D(x1 + np.array([0.00, 0.25]), [0.08, 0.08],
                                transform=fig.transFigure,
                                color='black',
                                linewidth=1.5,
                                zorder=0)
        fig.lines.append(line)
        fig.text(x1 + 0.25 * 0.5,
                 0.08,
                 'Timeline [years]',
                 ha='center',
                 va='bottom',
        ...
        for array in plans[iPlan].arrays:
            arrayTime = mpl.patches.Ellipse(
                (x1 + 0.25 * time_array[array.iTime] / np.max(time_array),
                 0.08),
                0.005,
                0.01,
                transform=fig.transFigure,
                ...
                zorder=2)
            fig.add_artist(arrayTime)
            fig.text(x1 + 0.25 * time_array[array.iTime] / np.max(time_array),
                     0.06,
                     '%i' % (time_array[array.iTime] / 365.25),
                     ha='center',
                     va='top',
        ...
        for sensor in plans[iPlan].sensors:
            arrayTime = mpl.patches.Ellipse(
                (x1 + 0.25 * times[sensor.install_timestep] / np.max(times),
                 0.08),
                0.005,
                0.01,
                transform=fig.transFigure,
